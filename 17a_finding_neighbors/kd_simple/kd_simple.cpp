...
using KDType=KDNode_<VectorType>;
using ContainerType=KDType::ContainerType;
using Scalar=typename VectorType::Scalar;
...
int main(int argc, char** argv) {
  ...
  Scalar range=100.f;
  ContainerType container(n_points);

  cerr << "generate samples" << endl;
  double t_start=getTime();
  for (auto& v: container){
    v=VectorType::Random()*range;
  }
  cerr << "time: " << getTime()-t_start << endl;
  
  KDType brute_force(container, container.size());

  cerr << "search (brute): " << endl;
  t_start=getTime();
  for (size_t i=0; i<container.size(); ++i) {
    auto bidx=brute_force.searchFast(container[i]);
    if (bidx!=i)
      throw std::runtime_error("error");
    // cerr << i << "->" << bidx << " " << container[i].transpose() << "->" << container[bidx].transpose() << endl;
  }
  cerr << "time: " << getTime()-t_start << endl;
  
  cerr << "build_tree (good)" << endl;
  t_start=getTime();
  KDType tree(container);
  cerr << "time: " << getTime()-t_start << endl;
  
  cerr << "search (tree): " << endl;
  t_start=getTime();
  for (size_t i=0; i<container.size(); ++i) {
    auto bidx=tree.searchFast(container[i]);
    if (bidx!=i)
      throw std::runtime_error("error");
    // cerr << i << "->" << bidx << " " << container[i].transpose() << "->" << container[bidx].transpose() << endl;
  }
