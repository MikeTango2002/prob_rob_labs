...
function [mu, sigma] = prediction(mu, sigma, control_input)
  ...
  A(1:3,1:3) = [1, 0, -u_x * s; #TODO: set portion that is affected by transition function
                0, 1, u_x * c;
                0, 0, 1];

  #Jacobian B: df(x, u)/du
  #for each state variable we have to associate the available control inputs
  B = zeros(dimension_mu, dimension_u); #TODO: set full control/state dimensions
  B(1:3,:) = [c, 0; #TODO: set portion that is affected by transition function
              s, 0;
              0, 1];
  ...
  sigma_u = [sigma_u^2 + sigma_T^2, 0; #TODO: set noise covariance
             0, sigma_u^2 + sigma_R^2];

  #predict sigma
  sigma = A * sigma * A' + B * simga_u * B'; #TODO: set full state sigma (consisting of state and control variables)
endfunction
