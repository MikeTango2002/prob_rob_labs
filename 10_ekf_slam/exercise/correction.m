...
function [mu, sigma, id_to_state_map, state_to_id_map] = correction(mu, sigma, observations, id_to_state_map, state_to_id_map)
  ...
  for i=1:M
    ...
    if(n != -1) 
      ...
      delta_t            = landmark_position - mu_t;  #TODO: relative landmark position
      measure_prediction = Rt * delta_t; #TODO: landmark position in robot
      ...
      C_m(1:2,1:2) = -Rt; #TODO: translation derivative dh(R,t)/dt
      C_m(1:2,3)   = Rtp * delta_t; #TODO: rotation derivative dh(R,t)/dR

      #jacobian piece w.r.t. landmark: dh/dx(n)
      C_m(:,id_state:id_state+1) = Rt;  #TODO: relative landmark measurement derivative
  ...
  if ((number_of_known_landmarks > 0))
    ...
    #observation noise
    noise   = 0.01;
    sigma_z = noise^2*eye(2 * number_of_known_landmarks); #TODO: set landmark noise dimensions
    ...
    #update sigma
    sigma = (eye(dimension_mu) - K*C) * sigma;  #TODO: set full state covariance dimension and complete covariance correction
  endif
endfunction
