...
function [mu, sigma, id_to_state_map, state_to_id_map] = addNewLandmarks(mu, sigma, measurements, id_to_state_map, state_to_id_map)
  ...
  for i=1:M
    ...
    if(id_to_state_map(id_landmark) == -1) 
      ...
      id_to_state_map(id_landmark) = n; #TODO: set landmark position in state
      state_to_id_map(n)           = id_landmark; #TODO: set landmark identifier
      
      #retrieve from the index the position of the landmark block in the state
      id_state = 4+2*(n-1);

      #compute landmark position in the world
      mark_position_in_robot = [measurement.x_pose;measurement.y_pose];
      landmark_position_in_world = mu_t + R*landmark_position_in_robot; #TODO: set position in world, recall: z(n)=Rt*(landmark_position_in_world-t)
