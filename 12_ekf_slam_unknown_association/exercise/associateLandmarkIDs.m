...
function observations = associateLandmarkIDs(mu, sigma, observations, state_to_id_map)
	...
	for n=1:N
		...
		# Già torna C[n], dove n = id_state
		# measurement_prediction è h[n]
    ...
    #for all landmarks
		for m=1:M
			...
			A(m, n) = (z - measurement_prediction)' * omega_nn * (z - measurement_prediction); #TODO likelihood with information matrix (omega)
		endfor
  	...
  	for m=1:M
		...
		if(a_mn < gating_tau)#TODO add gating condition for this association
			...
			#add the possible association - creating the associations vector
      		#[measurement id, proposed landmark id , association matrix value a_mn] 
			associations(end+1,:) = [m, min_index, a_mn]; #TODO add the information to our bookkeeping
			#associations = [associations; [m, min_index, a_mn]];
		endif
  	endfor
  ...
  number_of_valid_associations = 0;
	if(M > 1)
		for i=1:number_of_gated_associations
			...
			if(second_row_min_value - a_mn < lonely_best_friend_gamma_threshold || second_col_min_value - a_mn < lonely_best_friend_gamma_threshold)#TODO check if it is NOT a lonely best friend
