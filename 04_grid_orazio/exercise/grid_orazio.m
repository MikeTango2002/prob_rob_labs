...
#---------------------------------- FILTERING ----------------------------------
  ...
  transition_probability_matrix = transitionModel(map, state_ground_truth(1), state_ground_truth(2), control_input)
  ...
  state_belief = zeros(map_rows, map_cols);
	for row = 1:map_rows
		for col = 1:map_cols
      state_belief += transition_probability_matrix .* state_belief_previous; % .* is the element wise product in octave
		endfor
	endfor
